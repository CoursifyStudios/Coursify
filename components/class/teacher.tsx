...
const TeacherClass: NextPage<TeacherClassType> = ({
	...
	const Header = () => (
		<div className="w-full brightness-hover group relative h-16  overflow-hidden rounded-xl">
			{!settings.compact && (
				<Image
					src={classData?.image ? classData.image : exampleImage}
					loading="eager"
					alt="Example Image"
					className="w-full  absolute inset-0 h-16 w-full object-cover object-center brightness-75 transition duration-300 group-hover:brightness-90"
					fill
					sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, (max-width: 1280px) 33vw, (max-width: 1536px) 25vw, 20vw"
				/>
			)}
			<div className="w-full absolute bottom-2 left-4 top-2 flex flex-col justify-center">
				<h3 className="w-full max-w-[15rem] truncate text-xl font-semibold text-gray-200 dark:text-gray-800">
					{classData.name}
				</h3>
			</div>
			<div className="w-full absolute bottom-2 right-4 top-2 flex items-center ">
				<h2
	...
	const GradesSection: NextPage<{ name: string; grade: number }> = ({
		...
		<div>
			<h4>{name}</h4>
			<div className="w-full flex items-center text-base leading-4">
				<LineCounter amount={grade} />
				<p className="w-full ml-2">{grade}%</p>
			</div>
		</div>
	);
		...
		<div className={`${className} flex w-[19rem] flex-col gap-3.5`}>
			...
			>
				<div className="w-full flex justify-between">
					<div>
						<h3 className="w-full text-lg font-bold">Statistics</h3>
						<p className="w-full text-xs">S2 - Q4</p>
					</div>
					<Dropdown
				...
				<div className="w-full absolute -inset-3 bg-backdrop/10 backdrop-blur-[3px] select-none grid place-items-center font-medium">
				...
				getDataInArray(classData.assignments).map((assignment, i) => {
					...
					return (
						<section
							...
							key={i}
						>
							<div className="w-full mr-3 grid h-9 w-9 min-w-[2.25rem] place-items-center rounded-full bg-gray-300 dark:bg-gray-200">
								{
							...
							<div className="w-full flex flex-col ">
								<h4 className="w-full max-w-[10.5rem] truncate text-sm font-medium">
									{assignment.name}
								</h4>
								<p className="w-full text-xs">12/24 students submitted</p>
							</div>
							<div
								tabIndex={-1}
								className="w-full ml-auto flex flex-col items-center text-xs"
							>
								{dueDate && (
									<>
										<div className="w-full mb-0.5 font-medium text-gray-700">
