...
export default function ScheduleComponent({schedule, classes}:{schedule: ScheduleData, classes: AllClassesResponse}) {
                        ...
                        (item, index) =>
                            ...
                            /* checks that item (a ScheduleInterface) matches to a class, (see the function), and that the name attribute on it is not null */
                                !item.specialEvent && ( //If the item is not a special event... ...fill the UI with the stuff...
                                    <Link
                                        key={index}
                                        className="flex items-center justify-between font-semibold grow"
                                        href={
                                    ...
                                    >
                                        ...
                                        {
                                            classes?.data?.find( /* Here we check that the schedule and classes match up, using a slightly different process to the one used by checkClassMatchesSchedule). */
                                                (v) =>
                                                    v.block == item.block &&
                            ...
                            (item.specialEvent && checkClassMatchesSchedule(item) && ( // If the schedule item is for a special event, fill the UI with custom event stuff
